...
from ctr_estimator import CTREstimatorFor2
...
def get_camp_v(datas_list1, datas_list2):
    ...
    train_data1 = DatasetV2(all_train_data1, id[0])
    camp_v1 = train_data1.statistics['ecpc']

    all_train_data2 = [item for sublist in datas_list2[0 : n-1] for item in sublist]
    train_data2 = DatasetV2(all_train_data2, id[1])
    camp_v2 = train_data2.statistics['ecpc']
    return camp_v1, camp_v2

def V1(datas_list1, datas_list2, camp_v1, camp_v2):
    #----------------- data -----------------
    train_data1 = DatasetV2(datas_list1[0], id[0]) 
    train_data1.shuffle()
    train_data2 = DatasetV2(datas_list2[0], id[1])
    train_data2.shuffle()
    weight = None
    #----------------- train -----------------
    ...
    for i in range(1, n):
        print("Turn " + str(i) + " ...")
        test_data1 = DatasetV2(datas_list1[i], id[0])
        test_data2 = DatasetV2(datas_list2[i], id[1])
        ctr_estimator = CTREstimatorFor2(train_data1, test_data1, train_data2, test_data2, id[0], id[1], camp_v1, camp_v2, weight = weight)
        ...
        train_data1 = ctr_estimator.winning_data1
        train_data1.shuffle()
        train_data2 = ctr_estimator.winning_data2
        train_data2.shuffle()
        #----------------- output -----------------
...
def V2(datas_list1, datas_list2, camp_v1, camp_v2):
    ...
    train_data1 = DatasetV2(all_train_data1, id[0])
    train_data1.shuffle()
    test_data1 = DatasetV2(datas_list1[n-1], id[0])

    all_train_data2 = [item for sublist in datas_list2[0 : n-1] for item in sublist]
    train_data2 = DatasetV2(all_train_data2, id[1])
    train_data2.shuffle()
    test_data2 = DatasetV2(datas_list2[n-1], id[1])

    train_data1.output_statistics()
    test_data1.output_statistics()
    train_data2.output_statistics()
    test_data2.output_statistics()
    print("Data loaded.")
    #----------------- train -----------------    
    ctr_estimator = CTREstimatorFor2(train_data1, test_data1, train_data2, test_data2, id[0], id[1], camp_v1, camp_v2)
